...
#include "morton_code_gen.h"

// Container for bounding box
template <typename SpaceT>
struct PBTBoundingBox
{
	glm::vec<3, SpaceT> min_bounds;
	glm::vec<3, SpaceT> max_bounds;
};
...
public:
    ...
    {
        return data[index].total_intensity;
    }
...
private:
    std::array<PBTNode<SpaceT, LightT>, max_lights * 2 - 1> data;
	...
	void print_item(size_t& light_index) const
	{
        ...
        std::cout << data[light_index].total_intensity << std::endl;
    ...
    void initialize_leaf_nodes(ml::SceneLights<max_lights> const& lights)
	{
        ...
        for (size_t light_index = 0; light_index < lights.get_num_lights(); ++light_index)
        {
            ...
            data[num_leaves - 1 + light_index].total_intensity = lights.data()[morton_index_code[light_index].first].color.r +
                lights.data()[morton_index_code[light_index].first].color.g +
                lights.data()[morton_index_code[light_index].first].color.b +
                lights.data()[morton_index_code[light_index].first].color.a;
        }
